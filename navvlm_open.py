...
# ============================================================
# nav_open: FINAL_ADJUST 两层门控
#   第 1 层 (进入 FINAL_ADJUST): 在 auto_detect 结果处理处用全局 presence 窗口门控 --
#         最近 _PRESENCE_WINDOW 次 presence 的 yes 率 >= _PRESENCE_RATIO 且
#         采样跨度 >= _PRESENCE_SUSTAIN 秒, 才接受 final_target 并切 FINAL_ADJUST。
#         (SCANNING / VLM_INQUIRY / PATROL_* 等状态检测到目标也必须过这一关)
#   第 2 层 (FINAL_ADJUST 内部转出 FINAL_FOLLOW): 用老逻辑, 与 presence 窗口无关 --
#         _lock_final_adjust_if_ready 只看: 待够 FINAL_ADJUST_TIME(30s) +
#         帧数 >= FINAL_ADJUST_MIN_FRAMES(6) + 到位(<=FINAL_ADJUST_LOCK_DIST) 才锁;
#         或待够 FINAL_ADJUST_FORCE_TIME(60s) 超时兜底。不读任何 presence。
# ============================================================
_PRESENCE_WINDOW = 10        # 看最近这么多次 presence 结果 (FINAL_ADJUST_DET_PERIOD=1s -> ~20s)   15 if other
_PRESENCE_SUSTAIN = 5.0      # 窗口采样跨度必须 >= 这么久(s) 才满足第 1 层门控
_PRESENCE_RATIO = 0.8       # 第 1 层门控 yes 率阈值 (>= 该比例才认为目标持续可见)
_presence_samples = []       # [(timestamp, is_present), ...] 仅保留最近 _PRESENCE_WINDOW+1 个

def _present_register(is_present, now):
    """喂入一次 auto_detect 的 presence 结果 (复用 vlm present-then-detect, 不再单独问)。"""
    _presence_samples.append((now, bool(is_present)))
    _over = len(_presence_samples) - (_PRESENCE_WINDOW + 1)
    if _over > 0:
        del _presence_samples[:_over]

def _present_ratio(now):
    """返回最近 _PRESENCE_WINDOW 次 presence 的 yes 率；样本不足返回 None。"""
    n = len(_presence_samples)
    if n < _PRESENCE_WINDOW:
        return None
    _win = _presence_samples[-_PRESENCE_WINDOW:]
    _yes = sum(1 for _, p in _win if p)
    return _yes / _PRESENCE_WINDOW

def _present_locked(now):
    """最近 _PRESENCE_WINDOW 次 presence yes 率 >= _PRESENCE_RATIO
    且窗口跨度 >= _PRESENCE_SUSTAIN 秒 -> 锁定。
    """
    _ratio = _present_ratio(now)
    if _ratio is None or _ratio < _PRESENCE_RATIO:
        return False
    _win = _presence_samples[-_PRESENCE_WINDOW:]
    return (now - _win[0][0]) >= _PRESENCE_SUSTAIN

def _present_reset(reason=""):
    """清空 presence 窗口 (锁定/退出 FINAL_ADJUST 时调用, 避免残留样本在后续状态误触发)。"""
    _presence_samples.clear()
...
def main():
    ...
    def _lock_final_adjust_if_ready(distance_to_nav):
        """FINAL_ADJUST 内部锁定 (终调转 FINAL_FOLLOW) -- 用老逻辑, 与 presence 窗口无关。

        判定: 满足以下任一即锁定转 FINAL_FOLLOW:
          (A) 正常锁定: 在 FINAL_ADJUST 已待够 FINAL_ADJUST_TIME(30s) 且
              累积有效检测帧数 >= FINAL_ADJUST_MIN_FRAMES(6) 且
              离导航终点够近 (distance_to_nav <= FINAL_ADJUST_LOCK_DIST=1.5m);
          (B) 超时兜底: 在 FINAL_ADJUST 已待够 FINAL_ADJUST_FORCE_TIME(60s),
              不论是否到位都强制锁定, 防止卡死。
        注意: 进入 FINAL_ADJUST 本身的门控 (yes率+sustain) 在 auto_detect 结果处理处,
              本函数只负责 FINAL_ADJUST 内部何时转出, 不读任何 presence 窗口。
        """
    ...
    while (not rospy.is_shutdown()) and \
        ...
        # (FINAL_ADJUST 锁定改用 auto_detect 的 presence 结果, 见 _present_register;
        #  不再单独发 open_yesno, 避免与 auto_detect 抢同一 VLM worker)
        ...
        if (not _reloc_giveup) and \
            ...
            if _reloc_phase is None:
                ...
                _reloc_phase = "stop"
                _present_reset("进入RELOC")  # 清 presence 窗口, 保证"scanning 连续 present"从本轮回零
                _reloc_t = time.time()
                ...
                if (_vlm_result is not None and
                    ...
                    if _res.error:
                        ...
                        # 复用 FINAL_ADJUST 同款 presence 窗口: 最近 _PRESENCE_WINDOW
                        # 次 reloc_presence 结果 yes 率 >= _PRESENCE_RATIO 且跨度 >= _PRESENCE_SUSTAIN
                        # 秒, 才允许挑出转 FINAL_ADJUST; 单次 present 不再直接挑出
                        # (满足"scanning 时连续 present 才进 adjust")。
                        _present_register(_present, time.time())
                        _ratio_now = _present_ratio(time.time())
                        _ratio_str = (f"{_ratio_now:.1%}"
                                      if _ratio_now is not None
                                      else f"{len(_presence_samples)}/{_PRESENCE_WINDOW}")
                        if _present:
                            if _present_locked(time.time()):
                                motion_thread.stop()
                                _reloc_phase = "detect_hold"
                                _reloc_t = time.time()
                                _finish_vlm_result(
                                    _res, "apply", "presence_yes_locked")
                                print(f"[RELOC][PRESENCE] 画面 yes 率达到 "
                                      f"{_ratio_str} (>= {_PRESENCE_RATIO:.0%}) 且 "
                                      f">= {_PRESENCE_SUSTAIN:.0f}s -> "
                                      f"停止旋转并检测 bbox 挑出 RELOC")
                            else:
                                _finish_vlm_result(
                                    _res, "apply", "presence_yes_window")
                                print(f"[RELOC][PRESENCE] 画面存在 "
                                      f"'{_det_tgt}' 但窗口未满足 "
                                      f"(yes率={_ratio_str}), 继续旋转")
                        else:
                            _finish_vlm_result(
                    ...
                    if _res.error:
                        ...
                        if _vb and len(_vb) == 4:
                            ...
                            if _t3d is not None:
                                ...
                                _present_reset("进入FINAL_ADJUST(reloc_detect)")
                                path = None
                                path_idx = 0
        ...
        if _vlm_result is not None and _vlm_result.kind == "auto_detect":
            ...
            # 全局 presence 窗口: 每轮 (所有状态) 只要有问 presence 就喂入样本,
            # 供 FINAL_ADJUST 进入/锁定复用 (不再限定 FINAL_ADJUST/SCANNING)。
            if _need_presence:
                _present_register(_present, time.time())
            # ===== 全局门控标志: 每轮询问后计算 yes 率 + 跨度, 供"进 FINAL_ADJUST"统一判定 =====
            _ratio_now = _present_ratio(time.time())
            _ratio_str = (f"{_ratio_now:.1%}"
                          if _ratio_now is not None
                          else f"样本不足({len(_presence_samples)})")
            _pres_span = 0.0
            if len(_presence_samples) >= _PRESENCE_WINDOW:
                _pres_span = time.time() - _presence_samples[-_PRESENCE_WINDOW][0]
            elif _presence_samples:
                _pres_span = time.time() - _presence_samples[0][0]
            _present_ready = _present_locked(time.time())
            print(f"\033[96m[nav_open][PRESENCE] yes率={_ratio_str} "
                  f"跨度={_pres_span:.1f}s "
                  f"(需 yes率≥{_PRESENCE_RATIO:.0%} 且 跨度≥{_PRESENCE_SUSTAIN:.0f}s) "
                  f"-> {'满足' if _present_ready else '未满足'}\033[0m")
            ...
            if _res.error:
                ...
                if _vb and len(_vb) == 4:
                    ...
                    if (_t3d is not None and
                            state == NavState.SCANNING and
                            not _present_ready):
                        # SCANNING(开局/reloc恢复扫描) + presence 窗口未满足:
                        # 完全不动 final 目标 —— 既不进 FINAL_ADJUST, 也不覆盖
                        # final_target / final_goal_nav / goal_source, 避免扫到
                        # 目标一闪而过就改掉导航终点 (与 RELOC spin 同款门控)。
                        print(f"\033[93m[nav_open][SCAN] 检测到目标但 presence "
                              f"窗口未满足 "
                              f"(yes率={_ratio_str}, 跨度={_pres_span:.1f}s), "
                              f"不改 final 目标, 继续扫描\033[0m")
                        vlm_latest = (_vdets, _vmasks)
                        debugger.record_vlm({
                            "stage": "scan_window_unmet",
                            "request_id": _res.request_id,
                            "submit_step": _res.submit_step,
                            "result_step": step,
                            "bbox_2d": list(_vb),
                            "bbox_2d_px": _bbox_px,
                            "target_3d": [float(v) for v in _t3d],
                            "presence_samples": len(_presence_samples),
                            "trigger": _trigger,
                        }, step=step)
                        _finish_vlm_result(_res, "apply", "scan_window_unmet")
                    elif _t3d is not None:
                        ...
                        if state == NavState.FINAL_ADJUST:
                            ...
                            # 全局门控满足 (yes率 + 跨度) 才接受目标并进入 FINAL_ADJUST
                            final_target = (_t3d[0], _t3d[2])
                            _target_accepted = True
                            final_goal_nav = None
                            goal_source = "vlm_det"
                        else:
                            # 全局门控: 不满足 sustain + yes率, 不进 FINAL_ADJUST,
                            # 也不改 final_target / final_goal_nav / goal_source。
                            print(f"\033[93m[nav_open][GATE] 检测到目标但 presence "
                                  f"窗口未满足 (yes率={_ratio_str}, 跨度={_pres_span:.1f}s), "
                                  f"不进 FINAL_ADJUST, 留在 {state.name}\033[0m")
                            debugger.record_vlm({
                                "stage": "adjust_gate_unmet",
                                "request_id": _res.request_id,
                                "submit_step": _res.submit_step,
                                "result_step": step,
                                "bbox_2d": list(_vb),
                            ...
                            _finish_vlm_result(_res, "apply", "adjust_gate_unmet")
                            vlm_latest = (_vdets, _vmasks)
                        if _target_accepted:
                            _final_plan_fail_cnt = 0
                            ...
                            if state in (
                                ...
                                # 仅当窗口满足 (进入此分支说明 _present_ready) 才切状态
                                path = None
                                path_idx = 0
                                final_adjust_t = time.time()
                                final_adjust_frames = 1
                                state = NavState.FINAL_ADJUST
                                _present_reset("进入FINAL_ADJUST(o_detect)")
                            vlm_latest = (_vdets, _vmasks)
        ...
        if (_det_now - _auto_det_t >= _det_period and
            ...
            _det_tgt = VLM_DETECT_TARGET_DEFAULT
            _trigger = "auto"
            # 全局门控: 每轮都问 presence (不再因 _presence_confirmed 跳过),
            # 这样 presence 窗口能在所有状态持续累积, 作为"进 FINAL_ADJUST"的统一标志。
            _need_presence = True
            _submit_vlm(
            ...
            if not _boot_scan_done and cur_pose is not None:
                if mode is not None and mode.name == "TRACKING":
                    if BOOT_SCAN_ENABLED:
                        ...
                        _present_reset("进入SCANNING(开局/reloc恢复)")
                        _boot_scan_done = True
                    else:
            ...
            if cur_pose is None:
                ...
                if bbox is None:
                    ...
                    t3d = extract_target_3d_from_snapshot(
                        ...
                        map_points=_map_fallback,
                        debugger=debugger,
                        debug_step=_vlm_dir_submit_step)
                    if t3d is not None:
